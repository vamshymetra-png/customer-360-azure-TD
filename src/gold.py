...
from config import GOLD_DIR, MART_DIR, RISKY_MERCHANT_CATEGORIES

def apply_rule_based_fraud_score(transactions):
    return (
        transactions
...
def run_gold(silver):
    ...
    transactions_scored = train_score_fraud_model(apply_rule_based_fraud_score(silver["transactions"]))
    ...
    for name, df in gold_outputs.items():
        df.write.mode("overwrite").parquet(str(GOLD_DIR / name))
        df.coalesce(1).write.mode("overwrite").option("header", True).csv(str(GOLD_DIR / f"{name}_csv"))

    for name, df in marts.items():
        df.write.mode("overwrite").parquet(str(MART_DIR / name))
        df.coalesce(1).write.mode("overwrite").option("header", True).csv(str(MART_DIR / f"{name}_csv"))

    return {**gold_outputs, **marts}
